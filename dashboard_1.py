...
# ... existing code ...
    ...
    # Dapatkan level wilayah dan nama wilayah dari service point yang dipilih
    selected_row = wdf[wdf['service_point'] == sp].iloc[0]
    level_wilayah = selected_row['Level_wilayah']

    if level_wilayah == 'kec':
        # Jika level kecamatan, tampilkan semua service point di kecamatan tersebut
        kecamatan = selected_row['kecamatan']
        filtered_sp = wdf[wdf['kecamatan'] == kecamatan]
        
        st.subheader(f"Daftar Service Point di Kecamatan {kecamatan}")
        
        # Buat bar chart untuk level kecamatan
        fig = go.Figure()
        
        # Tambahkan bar untuk Kesehatan
        fig.add_trace(go.Bar(
            name='Kesehatan',
            x=filtered_sp['service_point'],
            y=filtered_sp['Kesehatan'],
            text=filtered_sp['Kesehatan'],
            textposition='auto',
        ))
        
        # Tambahkan bar untuk Pekerjaan Umum Dan Penataan Ruang
        fig.add_trace(go.Bar(
            name='Pekerjaan Umum Dan Penataan Ruang',
            x=filtered_sp['service_point'],
            y=filtered_sp['Pekerjaan Umum Dan Penataan Ruang'],
            text=filtered_sp['Pekerjaan Umum Dan Penataan Ruang'],
            textposition='auto',
        ))
        
        fig.update_layout(
            title=f'Jumlah Izin Kesehatan dan PU-PR di Kecamatan {kecamatan}',
            xaxis_title='Service Point',
            yaxis_title='Jumlah Izin',
            height=400,
            xaxis_tickangle=-45,
            barmode='group'
        )
        ...
        # Jika level kota, tampilkan hierarki kota -> kecamatan -> service point
        kota = selected_row['kota']
        kota_sp = wdf[wdf['kota'] == kota]
        
        st.subheader(f"Daftar Service Point di {kota}")
        
        # Buat bar chart untuk setiap kecamatan
        for kec in kota_sp['kecamatan'].unique():
            st.write(f"**Kecamatan {kec}**")
            
            kec_data = kota_sp[kota_sp['kecamatan'] == kec]
            ...
            fig.add_trace(go.Bar(
                name='Kesehatan',
                x=kec_data['service_point'],
                y=kec_data['Kesehatan'],
                text=kec_data['Kesehatan'],
                textposition='auto',
            ))
            ...
            fig.add_trace(go.Bar(
                name='Pekerjaan Umum Dan Penataan Ruang',
                x=kec_data['service_point'],
                y=kec_data['Pekerjaan Umum Dan Penataan Ruang'],
                text=kec_data['Pekerjaan Umum Dan Penataan Ruang'],
                textposition='auto',
            ))
            
            fig.update_layout(
                title=f'Jumlah Izin Kesehatan dan PU-PR di Kecamatan {kec}',
                xaxis_title='Service Point',
                yaxis_title='Jumlah Izin',
                height=400,
                xaxis_tickangle=-45,
